/**
 * Discord Bot Image Upload Handlers
 * Contains fixed implementations of the image upload handlers
 */
...
import { uploadImageUrlToImgur } from '../utils/imgurUploader';
 ...
 */
export async function collectImageMessage(channel: DMChannel | TextChannel | NewsChannel, filter: (m: Message) => boolean, timeoutMs: number = 300000): Promise<Message | null> {
 ...
 */
export async function handleInstaImageCommand(interaction: any) {
 ...
 */
export async function handleWebImageCommand(interaction: any) {
  await interaction.deferReply({ ephemeral: true });
 ...
 */
export async function handleInstaImageUploadButton(interaction: any, articleId: number, article: Article) {
  ...
  if (interaction.channel && interaction.channel.type === ChannelType.DM) {
    ...
    try {
      ...
      uploadChannel = interaction.channel as TextChannel | NewsChannel;
    }
  }
  
  if (!uploadChannel) {
    await interaction.editReply({
      content: `Unable to create upload channel. Please try uploading through the web dashboard instead.`,
 ...
 * This is part of the handleButtonInteraction function
 */
export async function handleWebImageUploadButton(interaction: any, articleId: number, article: Article) {
  // View in Dashboard button as fallback
  const viewInDashboardButton = new ButtonBuilder()
    .setLabel('View in Dashboard')
    .setStyle(ButtonStyle.Link)
