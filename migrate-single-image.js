/**
 * Migration Script for a single Airtable MainImage Attachment to MainImageLink Field
 * Optimized for extremely conservative usage to avoid rate limits
 */
...
const PROGRESS_FILE = './migration-single-image.json';
...
/**
 * Find the next record to process
 */
async function findNextRecordToProcess(records) {
  // Find the first record that has a MainImage but hasn't been processed yet
...
async function migrateSingleImage() {
  console.log('Starting single image migration from MainImage to MainImageLink');
  ...
  // Find next record to process
  const nextRecord = await findNextRecordToProcess(records);
  
  if (!nextRecord) {
    console.log('No more records to process! Migration completed!');
    console.log(`Total processed: ${progress.processedRecords.length} / ${records.length}`);
    return;
  }
  
  console.log(`Found record to process (${progress.processedRecords.length + 1} of ${records.length})`);
  
  // Process a single record
  const success = await processSingleRecord(nextRecord);
  
  console.log(`\nSingle record processing ${success ? 'completed successfully' : 'failed'}`);
  ...
  if (progress.processedRecords.length < records.length) {
    console.log(`\nRun this script again to process the next record.`);
  } else {
...
// Run the migration
migrateSingleImage().catch(error => {
