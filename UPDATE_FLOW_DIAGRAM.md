# Update Flow Diagram

This document provides visual representations of how updates flow through the PISCOC1 CMS system.

---

## System Architecture Overview

```
┌─────────────────────────────────────────────────────────────────────┐
│                          PISCOC1 CMS (This System)                   │
│  ┌──────────────────┐         ┌──────────────────┐                  │
│  │   React Client   │◄───────►│  Express Server  │                  │
│  │   (Frontend UI)  │         │   (API Backend)  │                  │
│  └──────────────────┘         └────────┬─────────┘                  │
│           │                             │                             │
│           │                             ▼                             │
│           │                    ┌──────────────────┐                  │
│           └───────────────────►│   PostgreSQL DB   │                 │
│                                 └──────────────────┘                  │
└─────────────────────────────────────────┬───────────────────────────┘
                                          │
                    ┌─────────────────────┼─────────────────────┐
                    │                     │                     │
                    ▼                     ▼                     ▼
            ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
            │   Website    │     │   Airtable   │     │  Instagram   │
            │ (External)   │     │  (Optional)  │     │  (Optional)  │
            └──────────────┘     └──────────────┘     └──────────────┘
```

---

## 1. Instant CMS Update Flow

**When:** Any content change (articles, team members, etc.)  
**Speed:** < 1 second

```
┌──────────┐                                              
│   User   │                                              
└────┬─────┘                                              
     │                                                    
     │ 1. Clicks "Save"                                  
     ▼                                                    
┌─────────────────┐                                      
│  React Client   │                                      
└────┬────────────┘                                      
     │                                                    
     │ 2. POST /api/articles                            
     ▼                                                    
┌─────────────────┐                                      
│ Express Server  │                                      
└────┬────────────┘                                      
     │                                                    
     │ 3. INSERT/UPDATE                                 
     ▼                                                    
┌─────────────────┐                                      
│  PostgreSQL DB  │                                      
└────┬────────────┘                                      
     │                                                    
     │ 4. Return updated data                           
     ▼                                                    
┌─────────────────┐                                      
│ Express Server  │                                      
└────┬────────────┘                                      
     │                                                    
     │ 5. Response with data                            
     ▼                                                    
┌─────────────────┐                                      
│  React Client   │                                      
└────┬────────────┘                                      
     │                                                    
     │ 6. queryClient.invalidateQueries()               
     │ 7. Refetch data                                  
     │ 8. Update UI                                     
     ▼                                                    
┌─────────────────┐                                      
│  UI Updates ⚡   │                                      
└─────────────────┘                                      
```

---

## 2. Webhook to Website Flow

**When:** Published article created/edited/deleted  
**Speed:** < 5 seconds

```
┌──────────┐                                              
│   User   │                                              
└────┬─────┘                                              
     │                                                    
     │ 1. Publishes article                              
     ▼                                                    
┌─────────────────┐                                      
│  React Client   │                                      
└────┬────────────┘                                      
     │                                                    
     │ 2. PUT /api/articles/:id                         
     │    { status: "published" }                        
     ▼                                                    
┌─────────────────┐                                      
│ Express Server  │                                      
│  routes.ts      │                                      
└────┬────────────┘                                      
     │                                                    
     │ 3. Update DB                                     
     ▼                                                    
┌─────────────────┐                                      
│  PostgreSQL DB  │                                      
└────┬────────────┘                                      
     │                                                    
     │ 4. Article updated                               
     ▼                                                    
┌─────────────────┐                                      
│ Express Server  │                                      
└────┬────────────┘                                      
     │                                                    
     │ 5. Call notifyArticlePublished()                 
     ▼                                                    
┌─────────────────┐                                      
│ webhookNotifier │                                      
└────┬────────────┘                                      
     │                                                    
     │ 6. POST to ARTICLE_WEBHOOK_URL                   
     │    { action: "published",                        
     │      articleId: 123,                             
     │      timestamp: "..." }                          
     ▼                                                    
┌─────────────────┐                                      
│    Website      │                                      
│   (External)    │                                      
└────┬────────────┘                                      
     │                                                    
     │ 7. Receives webhook                              
     │ 8. Refreshes cache                               
     │ 9. Updates content                               
     ▼                                                    
┌─────────────────┐                                      
│ Website shows    │                                      
│ new article ⚡   │                                      
└─────────────────┘                                      
```

---

## 3. Scheduled Publication Flow (with Airtable)

**When:** Article scheduled for future publication  
**Speed:** 0-60 second delay after scheduled time

```
┌──────────┐                                              
│   User   │                                              
└────┬─────┘                                              
     │                                                    
     │ 1. Creates article with                           
     │    Scheduled: "2025-11-23 14:30"                  
     │    Status: "pending"                              
     ▼                                                    
┌─────────────────┐                                      
│  PostgreSQL DB  │                                      
│  (Article saved)│                                      
└─────────────────┘                                      
                                                          
          ... Time passes ...                            
          Current time: 14:30:45                         
                                                          
┌─────────────────┐                                      
│   Scheduler     │ ◄── Runs every 60 seconds            
│  (Background)   │                                      
└────┬────────────┘                                      
     │                                                    
     │ 2. Checks for articles where                     
     │    Scheduled <= NOW                               
     ▼                                                    
┌─────────────────┐                                      
│  PostgreSQL DB  │                                      
└────┬────────────┘                                      
     │                                                    
     │ 3. Found article (id: 123)                       
     ▼                                                    
┌─────────────────┐                                      
│   Scheduler     │                                      
└────┬────────────┘                                      
     │                                                    
     │ 4. ensureArticleOnAirtable()                     
     ▼                                                    
┌─────────────────┐                                      
│    Airtable     │                                      
│  (If configured)│                                      
└────┬────────────┘                                      
     │                                                    
     │ 5. Article synced                                
     ▼                                                    
┌─────────────────┐                                      
│   Scheduler     │                                      
└────┬────────────┘                                      
     │                                                    
     │ 6. publishArticle()                              
     │    - Update status to "published"                
     │    - Set publishedAt to NOW                      
     ▼                                                    
┌─────────────────┐                                      
│  PostgreSQL DB  │                                      
└────┬────────────┘                                      
     │                                                    
     │ 7. Article updated                               
     ▼                                                    
┌─────────────────┐                                      
│   Scheduler     │                                      
└────┬────────────┘                                      
     │                                                    
     │ 8. notifyArticlePublished()                      
     ▼                                                    
┌─────────────────┐                                      
│    Website      │                                      
│   (Webhook)     │                                      
└────┬────────────┘                                      
     │                                                    
     │ 9. Website updates                               
     ▼                                                    
┌─────────────────┐                                      
│ Article appears  │                                      
│ on website ⏱️    │                                      
│ (0-60s delay)   │                                      
└─────────────────┘                                      
```

---

## 4. Update Types Comparison

```
┌─────────────────────┬──────────┬─────────────┬─────────────────────┐
│   Update Type       │  Speed   │  Triggers   │  Affects Website    │
├─────────────────────┼──────────┼─────────────┼─────────────────────┤
│ CMS UI              │ ⚡ < 1s   │ All changes │ No (CMS only)       │
│                     │          │             │                     │
│ Webhook to Website  │ ⚡ < 5s   │ Published   │ ✅ Yes - Instant    │
│                     │          │ articles    │                     │
│                     │          │ only        │                     │
│                     │          │             │                     │
│ Scheduled Publish   │ ⏱️ 0-60s │ Scheduled   │ ✅ Yes - After      │
│ (via Scheduler)     │          │ date/time   │    0-60s delay      │
│                     │          │             │                     │
│ Manual Airtable     │ 🔄 On    │ Admin       │ No (unless website  │
│ Sync                │ demand   │ trigger     │ uses Airtable)      │
│                     │          │             │                     │
│ Instagram Post      │ ⚡ < 10s  │ Article     │ No (separate        │
│                     │          │ published   │ platform)           │
└─────────────────────┴──────────┴─────────────┴─────────────────────┘
```

---

## 5. Decision Tree: Will My Change Update the Website?

```
                    ┌───────────────────┐
                    │  Made a change    │
                    │  in the CMS       │
                    └─────────┬─────────┘
                              │
                              ▼
                    ┌───────────────────┐
                    │  Is it an         │
              ┌─────┤  article?         ├─────┐
              │     └───────────────────┘     │
              │ Yes                       No  │
              ▼                               ▼
    ┌───────────────────┐         ┌────────────────────┐
    │  Is the article   │         │  Changes only      │
    │  published?       │         │  visible in CMS    │
    └─────────┬─────────┘         │  (not on website)  │
              │                   └────────────────────┘
        ┌─────┴─────┐
        │ Yes   No  │
        ▼           ▼
┌──────────────┐ ┌────────────────────┐
│ ✅ Website   │ │ ❌ Website won't   │
│ will update  │ │ update until       │
│ via webhook  │ │ article is         │
│ (< 5s)       │ │ published          │
└──────────────┘ └────────────────────┘
                         │
                         │ Future publication
                         │ date set?
                         │
                    ┌────┴────┐
                    │ Yes  No │
                    ▼         ▼
            ┌──────────┐ ┌──────────┐
            │ ✅ Will  │ │ ❌ Won't │
            │ publish  │ │ publish  │
            │ when     │ │ until    │
            │ scheduled│ │ you      │
            │ (0-60s   │ │ manually │
            │ delay)   │ │ publish  │
            └──────────┘ └──────────┘
```

---

## 6. Troubleshooting Flow

```
┌───────────────────────┐
│ Website not updating? │
└──────────┬────────────┘
           │
           ▼
┌───────────────────────┐
│ Check: Is article     │
│ status "published"?   │
└──────────┬────────────┘
           │
    ┌──────┴──────┐
    │ No      Yes │
    ▼             ▼
┌─────────┐  ┌──────────────────┐
│ Publish │  │ Check: Is        │
│ article │  │ ARTICLE_WEBHOOK_ │
│ to      │  │ URL configured?  │
│ trigger │  └────────┬─────────┘
│ update  │           │
└─────────┘    ┌──────┴──────┐
               │ No      Yes │
               ▼             ▼
          ┌─────────┐  ┌──────────────┐
          │ Set env │  │ Check server │
          │ variable│  │ logs for     │
          │ and     │  │ webhook      │
          │ restart │  │ errors       │
          └─────────┘  └──────┬───────┘
                              │
                       ┌──────┴──────┐
                       │ Errors? No  │
                       ▼             ▼
                  ┌────────┐    ┌─────────────┐
                  │ Check  │    │ Check if    │
                  │ webhook│    │ website is  │
                  │ URL is │    │ processing  │
                  │ correct│    │ webhooks    │
                  └────────┘    └─────────────┘
```

---

## 7. Data Flow Between Systems

```
                    ┌─────────────────────────────────┐
                    │      PISCOC1 CMS (Source)       │
                    │                                 │
                    │  ┌──────────────────────────┐   │
                    │  │    PostgreSQL Database   │   │
                    │  │  (Single source of truth)│   │
                    │  └───────────┬──────────────┘   │
                    └──────────────┼──────────────────┘
                                   │
                    ┌──────────────┼──────────────┐
                    │              │              │
                    │              │              │
        ┌───────────▼──────┐      │      ┌───────▼──────────┐
        │   Airtable       │      │      │   Website        │
        │   (Optional      │      │      │   (Consumes      │
        │    Secondary     │      │      │    via API or    │
        │    Storage)      │      │      │    Webhooks)     │
        └──────────────────┘      │      └──────────────────┘
                                  │
                          ┌───────▼──────────┐
                          │   Instagram      │
                          │   (Publishes     │
                          │    Content)      │
                          └──────────────────┘

Flow Direction:
  CMS → Airtable:    Scheduled sync (60s) or manual
  CMS → Website:     Webhook on publish (instant)
  CMS → Instagram:   Auto-post on publish (instant)
  
Note: Website pulls from CMS, not from Airtable
```

---

## 8. Webhook Payload Structure

**Webhook payload example:**
```json
{
  "action": "published",
  "articleId": 123,
  "timestamp": "2025-11-23T21:00:00.000Z"
}
```

**Possible action values:**
- `"published"` - Article was just published
- `"edited"` - Published article was edited
- `"deleted"` - Published article was deleted

**HTTP Request:**
```
POST https://www.pinkytoepaper.com/api/webhooks/article-published
Content-Type: application/json
User-Agent: PISCOC1-Webhook/1.0

{
  "action": "published",
  "articleId": 123,
  "timestamp": "2025-11-23T21:00:00.000Z"
}
```

---

## Summary

- **CMS Changes**: Always instant in the CMS UI
- **Website Updates**: Instant for published articles (via webhooks)
- **Draft Changes**: Not visible on website until published
- **Scheduled Articles**: Published 0-60 seconds after scheduled time
- **Airtable**: Used for backup/sync, not primary website data source

The key takeaway: **Publish your articles to make them visible on the website instantly!**
